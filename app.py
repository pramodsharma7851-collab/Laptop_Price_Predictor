...
# screen_size=st.number_input('screen Size')
# #resolution
# resolution = st.selectbox('Screen Resolution',['1920x1080','1366x768','1600x90','3840x2160','3200x1800','2800x1800','2560x1600','2304x1440'])
#
# # cpu brand
#
# CPU =st.selectbox('CPU',df['Cpu_brand'].unique())
# # cpu_speed =st.selectbox('Cpu speed',df['cpu_speed'].unique())
# cpu_speed  =st.number_input('CPU speed')
# hdd=st.selectbox('HDD(in GB)',df['HDD'].unique())
# ssd=st.selectbox('SSD(in GB)',df['SSD'].unique())
# GPU=st.selectbox('GPU',df['Gpu_brand'].unique())
...
# Y_res =int(resolution.split('x')[1])
# PPI=((X_res**2 +Y_res**2)**.5)/(screen_size+.0001)
# # query = np.array([company,TypeName,Ram,Weight,TouchScreen,Ips,PPI,CPU,cpu_speed,hdd,ssd,GPU,Operating_system])
